...
async function beginKadena() {
  ...
  for (const index of stringOfTenChars) { // async inside
    ...
    const kdaNodesValid = kdaNodes.filter((node) => (node.hash === 'localSpecificationsVersion11' || node.hash === 'localSpecificationsVersion10'));
    const kdaNodesINValid = kdaNodes.filter((node) => (node.hash !== 'localSpecificationsVersion11' && node.hash !== 'localSpecificationsVersion10'));
